...
test('deployable sources exclude private recruiting evidence', async () => {
  ...
  const forbidden = [
    /[REDACTED_TARGET_COMPANY]|[REDACTED]|[REDACTED_TARGET_COMPANY]|[REDACTED]|[REDACTED_TARGET_COMPANY]|hanwha\s*life/i,
    /(?:low|high)[ -]?fit|[REDACTED_FIT_ASSESSMENT]|AI Backend\s*82|JD\s*(?:score|적합도)/i,
    /private-harness/i,
    /(?:^|[^A-Za-z0-9])(?:kh|gp|gd)(?:[^A-Za-z0-9]|$)/i,
    /548\s*(?:files|파일)|3,347\s*(?:chunks|청크)|109\s*(?:chunks|청크)/i,
    /15\/15\s+default-on|private_search_tool|RAG\s+default-on/i,
    /873\s*files|6,212\s*chunks|6,809\s*nodes|6,720\s*edges|56,072\s*chunks|1,849\s*nodes|8,037\s*edges|120\s*chunks/i,
    /[REDACTED_PRIVATE_INVENTORY]|[REDACTED_PRIVATE_INVENTORY]/i,
  ];
